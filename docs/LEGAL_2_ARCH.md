...
- `closed` = `finalized` **AND** 모든 필수 조치의 이행·효과확인 완료 **AND**(KR 관할 한정) **현재 평가·조치 상태와 일치하는 사후 공유 기록**이 존재.
  - 근거: 시행규칙 제37조의4는 **공유사항을 포함한 기록**을 요구한다 → 사후 공유 기록이 없으면 그 평가의 기록은 아직 완결되지 않은 것으로 본다(LEGAL-2d §4, 오너 승인 2026-07-18).
  - ⚠️ **이것은 법적 준수 판정이 아니라 "워크플로우 필수 기록 완전성" 조건이다.** 앱은 "위반"이라 말하지 않고 "이 워크플로우가 요구하는 기록이 아직 없음"만 표시한다(§1.1 상단 교정 #6과 동일한 원칙).
  - 개선조치가 바뀌면 이전 사후 공유는 **현재 상태 공유가 아니게 되어**(stale) 다시 미종결이 된다. 과거 기록은 이력에 보존된다.
  - **US·관할 미설정에는 이 조건을 강제하지 않는다** — 기존 규칙(finalized + 필수 조치 완료) 그대로. 코드: `AssessmentClosure.isClosed` → `SharingEventPolicy.satisfiesPostSharingGate`.
