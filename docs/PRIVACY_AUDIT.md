...
| **② 한국 PIPA** | 제3자 개인정보 처리의 **권한·고지·보존·삭제 책임** | 참여자(근로자) 이름·서명 = 제3자 → PIPA 부담 ↑. **사업주가 처리자일 가능성 높음**; 앱 제공자의 수탁자 해당 여부는 **계약·실제 접근/처리 구조에 따라 별도 법률 검토**(PIPA 제26조 — 단정 금지). **ASC 판정과 무관** |
...
- 참여자·서명 = 제3자 개인정보 → **입력 권한·고지·보존·삭제·열람** 책임. **사업주가 처리자일 가능성 높음**; 앱 제공자의 수탁자(PIPA 제26조) 해당 여부는 **미확정 — 계약·실제 개인정보 접근/처리 구조에 따라 별도 법률 검토 필요**(개발자가 private CloudKit 데이터에 접근하지 않는 현 구조에서 수탁 성립 단정 불가).
