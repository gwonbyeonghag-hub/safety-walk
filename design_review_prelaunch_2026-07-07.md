...
> **[해소 기록 2026-07-08]** F-1 ✅ 해소 — 실기기 첫 실행 정상(시뮬 전용 확정) + 원인 확진(CloudKit 컨테이너 init; in-memory 대조실험) + **WO-9**(샌드박스+복구 팩토리, `fatalError` 제거)로 초기화 시뮬에서도 온보딩 정상 표시. F-2 = 오탐(WO-7 검수 때 이미 수정됨, main 366행 `homeNoDueAssessments`). F-3 ✅ 아이콘 교체(안전모, iOS 라·다·틴티드+macOS). 잔여 = F-4·F-5(fast-follow).
