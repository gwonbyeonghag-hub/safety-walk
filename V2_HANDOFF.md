...
```
- `riskLevel`은 **입력 완료 시에만** resolved(빈도×강도 두 값 모두 입력, 3단계 직접 선택). **미입력=nil, 0으로 대체 계산 금지**(LEGAL-0). 저장은 전 항목 입력 완료 시에만.
...
## WO-6b — 제출규격 스크린샷 캡처 패스 ✅ 완료·검수 통과·머지 (2026-07-10)

> **플래너 검수**: 13파일 픽셀 직접 실측 = 전량 규격 정확(1320×2868 ×9 · 2064×2752 ×1 · 2880×1800 ×4) ✅ · 스샷 육안(populated·WO-11 새 대시보드·위험색) ✅ · MacScreenshotEvidence 하네스 = 전체 #if DEBUG + opt-in 플래그 + 근거 문서화 → **잔존 승인**(재사용 캡처 도구, Release 스트립) ✅ · 좌표클릭 1회 위반 = 정직 보고 수용, 규칙 재확인.
> **iPad RA 시트 미오픈 = 플래너가 서명 빌드+접근성 탭 프로브로 재현 → 실제 버그 확정(WO-12 등재)**. 검수 중 별건 규명: 플래너의 CODE_SIGNING_ALLOWED=NO 무서명 빌드는 iCloud 엔타이틀먼트 부재로 CloudKit 미러링이 SIGTRAP — 서명 빌드는 정상(검증 빌드 시 서명 유지할 것).
...
# 🎯 스코프 재편 (오너 확정 2026-07-13) — "전문성 = 구색 다 갖춰 출시"

오너 판단: 지금 앱이 "바이브코딩 같다"는 인상 우려 → **검색·달력·통계를 풀기능으로 넣고 출시**(월 단위 연기 감수). 법인/팀(계정·권한·공유·감사)만 v3로 제외.
**게이팅**: 검색=무료 · **통계·달력=Pro**(기존 SafetyWalk Pro 그룹에 편입, 신상품 없음). 차트=Swift Charts(내장, 의존성 0). 데이터 필드(startedAt·assessedAt·createdAt·status·riskLevel·siteId) 전부 존재 확인.
**⚠️ 순서 규율(중요)**: WO-15(Mac Table 대표) → **오너 실화면 승인** → WO-16 확산 → 그 다음에 검색/통계/달력. 각 큰 기능은 **스펙 먼저 못 박고**(범위 경계 = v3 안 넘게) 착수. Connect 앱 등록·구독은 병행하되 **최종 제출은 전부 끝난 뒤**.

## 출시 범위 WO 목록 (순서대로)
- **WO-15** Mac 위험요인 Table 대표구현 (등재됨) → 승인 게이트
- **WO-16** Mac 나머지 Browse 3개 확산 (등재됨)
- **WO-17** 접근성 F-4/F-5 마감 (등재됨)
- **WO-18** Mac 대시보드 레이아웃 교정 (등재됨) — 통계 대시보드와 통합 검토
- **WO-19** 🔍 통합 검색 (무료) — iOS+Mac, 전 엔티티(현장·점검·위험요인·위험성평가) 교차 검색 화면. 결과 유형별 그룹·탭 이동. 순수 검색 로직 분리+TDD. 쓰기/설정 검색 X.
- **WO-20** 📊 통계 대시보드 (Pro) — Swift Charts. 지표: 기간별 점검 수·미조치 위험 추이·현장별 분포·위험등급 비율·적합률. 기간 필터(주/월/분기). Pro 게이트. **집계 순수함수 TDD**(경계·빈데이터). PDF 내보내기는 v3.
- **WO-21** 📅 달력 뷰 (Pro) — 점검·위험성평가 이력을 월 달력에 **읽기 표시**(일자 탭→해당일 목록). 일정 잡기/알림=v3(쓰기 금지). Pro 게이트.
- **마감** Mac 스샷 재선정·촬영(Table·교정 대시보드·통계) → 메타데이터에 신기능·Pro 반영 → 제출.

## v3로 명시 제외 (스코프 크리프 차단)
...
## 순서 (확정 — 순환 모순 제거, 검토자 교정 2026-07-13)
```
...
# ⚖️ 법규 정합 트랙 (단일 정본 · 검토자 2차 반영 2026-07-13) — 위 구안 제거됨

플래너 계획의 결함 5건을 검토자가 지적 → 전면 수용. 아래가 **최종 확정판**(위 LEGAL-0~6 블록보다 우선).

## LEGAL-0 설계 교정 — enum 추가 ❌ → optional ✅ (마이그레이션 회피)
`RiskLevel.notAssessed` 케이스 추가는 **SwiftData 마이그레이션 유발**이라 기각. 대신:
- `DraftItem.directRiskLevel: RiskLevel? = nil` · `resolvedLevel() -> RiskLevel?`
- 3단계: 사용자가 위험도 **직접 선택해야 저장**. 빈도×강도: 두 값 모두 있어야 저장. 미입력 시 색·점수·등급 **미표시**.
- **자동 Medium도 제거**: `RiskAssessmentViewModel.swift:109 d.directRiskLevel = .medium`(체크리스트 부적합→연결위험 없을 때 자동 보통) → 사용자 확인 없이 확정 위험도 저장 금지(nil 또는 명시 확인 상태).
- **모델 마이그레이션 0.** 미완성 임시저장이 필요해지면 그때 별도 draft/final 상태 설계.

## 순서 재배치 — 법규를 검색/통계/달력 "앞"으로 (재작업 방지)
관할·업종·참여자·**조치이행·개선후위험도(모두 LEGAL-2)** 필드가 나중에 바뀌면 검색인덱스·통계집계·Mac표를 재작업. → **도메인 모델 먼저 안정화.**
```
LEGAL-QUICK  TBM 키워드 제거 + "정식" 문구 완화 (즉시)
LEGAL-0      위험도 입력 안전성 (optional, 자동Low+자동Medium 제거)
LEGAL-1      관할·업종 분리 + 평가에 기준 스냅샷 저장
[게이트]      한국/미국 요구사항 출처표 + 전문가 검토 (임의 해석 금지)
LEGAL-2      한국 평가·**조치이행·개선후위험도**·참여·공유·3년보존 (+ 개인정보 게이트 ↓)
LEGAL-3      미국 JHA 재검토 + 일반산업/건설 템플릿 분리 + State Plan 경고
LEGAL-4/TBM  TBM 실모듈 구현 (TBM-0~4, 오너 확정 "무조건 구현")
LEGAL-5      최종 전문가 검토·문구·스샷·제출
──── 도메인 안정 후 ────
WO-A/B/C(결제, 별도 워크스페이스 권장) · WO-15~18(Mac) · WO-19(검색)/20(통계)/21(달력)
```
> ⚠️ 결제(WO-A/B/C)는 병행 가능하되 **같은 공유 워킹카피 동시 병렬 지양**(브랜치 충돌).

## 🔴 신규 게이트 — 개인정보 재검토 (검토자 캐치, 플래너 누락했던 것)
LEGAL-2에서 **근로자·근로자대표 이름을 CloudKit에 추가하면 App Privacy "Data Not Collected" 결론이 깨질 수 있음.** WO-6 개인정보 문서(privacy_policy·app_privacy_answers) 재검토 필수. **미리 이름 빼지 말 것**(시행규칙 37조의4가 참여자 기록 요구 → 성급 제거 시 기록요건 약화). 필수/선택은 아래 개인정보 게이트에서 확정. 기존 "수집 0" 자동 유지 금지.

## 목표 문구 정정
"법규 완전 정합" ❌ → **"한국 현행 위험성평가 절차 + 미국 연방 OSHA 기준으로 범위 한정, 워크플로·기록항목 정렬, 전문가 검토 참고도구"**. 미국 JHA=연 1회 법정의무 아님(변경·사고·아차사고·주기검토 권고). State Plan은 연방보다 상이/엄격 가능 — 경고 표시. **법률자문·준수인증 아님 유지.**

## TBM — 검토자 #4 + 오너 확정 결합
- 출시 전: **키워드/기능주장 제거**(부정확 메타데이터 위험, LEGAL-QUICK).
- **TBM 실모듈 = 오너 "무조건 구현" 확정 → 정식 제품기능으로 편입**(참석자·전달내용·확인·보고서). 법적 출시조건은 아니나 제품 로드맵 필수 항목. 관할별 요구(연방 전력작업·California 등) 반영은 신뢰출처 게이트 후.

## 플래너 문서 사고 정정 (검토자 #6)
- `docs/LEGAL_READINESS_KR_US.md` 미추적이었음 → 이 커밋에 함께 등재. 날짜 오타 `2025-07-13`→`2026-07-13` 수정.
- 법령 기준일: 한국 산안법 36조/시행규칙 **2026-06-01 시행분**(과태료 부과 50인↑ 2027-01-01). 미국 1910.132(d)·1926.501. (플래너 웹 대조 확인, 2026-07-13.)

## LEGAL-0 완료 조건 (확장 — 검토자 2차)
optional만으로 불완전. 다음 전부 포함:
- [ ] `DraftItem.directRiskLevel: RiskLevel?`(nil), `resolvedLevel() -> RiskLevel?` — 미입력 시 색·점수·등급 미표시.
- [ ] **영속 생성자 필수화**: `RiskAssessmentItem.init`의 `riskLevel: RiskLevel = .low` 기본값 제거 → **명시적 riskLevel 필수 인자**(스키마 저장 프로퍼티 기본값은 SwiftData 위해 남기되 생성자는 강제).
- [ ] **자동 Medium 제거**: `RiskAssessmentViewModel:109 d.directRiskLevel = .medium`(체크리스트 부적합 시드) → nil 또는 사용자 확인 상태. **연결 위험요인 기존 등급도 "참고값"으로만 표시, 사용자 확인 필요**(자동 확정 금지).
- [ ] **전체 항목 검증**: 모든 draft 항목의 위험 입력이 완성돼야 전체 저장 가능(canSave 강화).
- [ ] **저장 실패 처리**: `RiskAssessmentViewModel:151 try? context.save()` → do/catch, 실패 시 **화면 유지 + 오류 안내**. `RiskAssessmentCreateView:64` 무조건 `dismiss()` → **저장 성공 시에만 dismiss**.
- [ ] 자동 Low·Medium 경로 테스트(/tdd) + 저장실패 경로.

## 🔴 개인정보 게이트 — 질문 교정 (검토자 #3: "이름 제거"가 아님)
한국 시행규칙 제37조의4가 기록에 **"참여 근로자·근로자대표"** 포함 요구 → **미리 이름 빼면 기록 요건 약화.** 게이트 질문:
- [ ] 신원 기록이 실제 필요한 범위(시행규칙 요구 수준) = **전문가 게이트에서 확정 후** 필수/선택 결정
- [ ] CloudKit 저장·삭제·내보내기를 개인정보처리방침에 어떻게 설명
- [ ] App Privacy "수집" 선언 갱신(근로자 이름 추가 시 "Data Not Collected" 재검토 — 자동 유지 금지)

## TBM 트랙 (단일 거대 WO ❌ → 5분할 · 검토자 #4 + 오너 "무조건 구현")
내부 공통 명칭 **Safety Briefing**. 표시: 한국=**TBM**, 미국=**Toolbox Talk/Pre-Task Briefing**. 법적 주기·규정 프로필 분리.
- **TBM-0**: 공식 출처·용어·개인정보·범위 확정(게이트). 한국=작업 전 안전점검회의 상시 주지 노력의무 / 미국=연방 공통주기 없음, 전력작업 1926.952·California 1509(10근무일) 등 관할별.
- **TBM-1**: 공통 SafetyBriefing 모델 + 마이그레이션(SWIFTDATA_MIGRATION.md). **당시 위험성평가 스냅샷 보존**(원본 수정돼도 과거 회의 불변). 참석확인 = "전달받음" 기록이지 **책임포기·안전동의 서명 아님**(기록도구 원칙).
- **TBM-2**: iPhone/iPad 생성·참석·의견·신규위험·후속조치.
...
> ✅ **완료·검증·병합 2026-07-13 (main b68f583, 15파일 +394/−49)** — 리뷰어 독립검증: 0a~0e 코드 정독 + DEBUG훅 `#if DEBUG` 게이트 확인 + save() 방어guard/rollback 확인, Core RA 13테스트·0f 5테스트 실행 그린, 스샷 3상태(미평가/저장차단/저장실패) 육안 확인, iOS/Mac 컴파일 전이확인(스샷 런·SeedData riskLevel 명시).
> ⚠️ **선재 플레이크 발견(LEGAL-0 무관)**: `SchemaMigrationTests` 전체 병렬 스위트에서 SwiftData 레이스로 SIGABRT — **main도 3/5 크래시**, `--no-parallel`이면 52/52 그린. 별도 태스크로 분리(테스트 직렬화).
...
---
# ✅ WO LEGAL-3A — 미국 업종 스냅샷 + 관할별 검토 알림 교정 — **병합 완료·검증 2026-08-09 (main 31ae6eb, fast-forward)**
> 스코프: ① `industryProfile`(일반/건설/전기) — US 관할일 때만 필수(`JurisdictionPolicy.requiresIndustry`), 자동선택 없음, `RiskAssessment.industryProfileSnapshot`으로 스냅샷·불변. 표시는 신규 공유 프로퍼티 `RiskAssessment.industryDisplayText`(nil→미기록/Not recorded) 하나로 통일해 iOS 상세·macOS 조회·KR·US PDF 네 소비처가 전부 이 값만 읽는다. ② 검토 알림을 관할 인식 정책으로 교정 — `dueStatus()`가 `kind == .regular && jurisdiction == .kr && assessedAt != nil`을 Core 안에서 전부 흡수(호출부 HomeView/DashboardView는 kind 재확인 없이 `dueStatus()`만 호출), KR 외 모든 관할(US·미설정)과 KR이라도 initial/occasional 종류는 365일이 지나도 자동 플래그 없음. ③ en/ko 현지화 6키 신설 + 기존 due 관련 8키 문구(원 WO 4개 + R1 4개)를 법적기한 어감 제거한 문구로 교정(PM 지정 값).
> 3라운드로 진행: (1) 원 WO `37919f8` — 업종 스냅샷+관할 인식 알림 최초 구현. (2) PM 반송 `9b3a5ee` — 초회 수정이 놓친 잔여 법적기한 어감 문구 4건 교정 + `dueStatus()`에 kind 흡수(화면별 정책 중복 제거) + `industryDisplayText` 공유 프로퍼티 신설(4 소비처 통일) + Core/앱 테스트 보강. (3) PM 반송 `31ae6eb` — R1에서 추가한 PDF "업종 미기록" 테스트 2건이 위양성 가능(다른 빈 필드도 같은 미기록 라벨을 낼 수 있어 배선 안 해도 통과 가능)이라고 지적받아, 다른 모든 미기록-가능 필드를 고유 비공백 값으로 채우고 `>= 1`→`== 1`로 강화, red-check(업종을 실값으로 임시 설정 → 두 테스트 모두 `("0") is not equal to ("1")`로 실패 확인 후 원복)로 비-위양성을 실측 증명. 앱 구현 코드 무변경(테스트 파일 1개만).
> 병합 후 검증(fast-forward, merge commit 없음): **Core `swift test` 327/327**(43 suites) · iOS·Mac 서명 빌드 둘 다 `BUILD SUCCEEDED` · `ReportRenderingTests` 16/16 개별 재확인(신규 업종 미기록 테스트 2건 포함) · `git diff --check main...HEAD` clean · 병합 전 working tree clean.
> 🟢 실행자 정직 보고(라운드별 자체 인정): R1 — 최초 구현이 due 관련 4키의 잔여 법적기한 문구를 놓친 것을 PM이 발견 → 즉시 교정. R2 — PDF 미기록 테스트 2건이 배선 여부와 무관하게 통과할 수 있는 구조였음을 PM이 지적 → red-check로 비위양성 실측 증명 후 반영.
> 📝 부수 발견(비블로커, 별도 기록): XCUITest `scrollToElement`가 `.exists`만 확인해 실제로는 화면 밖(hittable 아님)인 요소에서 오탐 — `isHittable` 병기로 대부분 해결됐으나 `WO5ScreensUITests` 1건은 근본적으로 항목 추가 순서(관할/업종 선택 전에 항목 3개를 먼저 추가) 문제였음 — `git worktree`로 클린 main과 격리 비교해 회귀임을 실측 확인 후 순서 재배치로 해결(재사용 가능한 회귀 격리 기법으로 기록).
> **iOS 홈/Mac 대시보드 due 관련 UI 카피 최종본**: `home.assessmentsDue`="Assessments to review"/"검토할 위험성평가", `home.noDueAssessments`="No assessments to review"/"검토할 위험성평가 없음", `mac.stat.assessmentsDue`="Reviews needed"/"검토 필요", `mac.assessmentsDue`="Risk assessments to review"/"검토할 위험성평가", `home.overdue`/`mac.overdue`="Review due"/"검토 필요", `home.dueSoon`/`mac.dueSoon`="Review soon"/"검토 시점 임박".
> **다음 LEGAL**: LEGAL-3B — 미국 JHA 필드 재검토, 일반산업/건설 템플릿 분리, Federal OSHA 범위 및 State Plan 경고 표시, 공식 출처 기반 prompt-by-prompt 검증. PPE 인증 워크플로우는 별도 명시 승인 전 제외.
