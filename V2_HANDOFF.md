...
# ⚖️ 법규 정합 트랙 (오너 확정 2026-07-13 — "7개 전부, 분기 각오")

근거: `docs/LEGAL_READINESS_KR_US.md`(검토자, 공식 출처 포함). **플래너 코드검증**: #1 자동Low(`RiskAssessmentViewModel.resolvedLevel` 58행 `return .low`, `directRiskLevel=.low`)·#3 저장평가 읽기전용(Detail 편집진입점 0)·#4 언어=관할 강결합(LocalizationManager 주석 "coupled")·#7 TBM 화면 0(메타 키워드만) = **코드로 확정**. #2·#5·#6 = 코드구조는 부합하나 **법 해석은 신뢰출처 확정 선행 필요**(플래너/실행자가 임의 해석 금지). 오너 결정 = **7건 전부 반영, 출시 분기 지연 감수**.

## ⛔ 진짜 위험 (경로 무관 즉시)
- **LEGAL-0 (BLOCKER)**: 미입력 위험 자동 `.low` 완전 제거 → **`.notAssessed` 도입**. RiskLevel에 케이스 추가(마이그레이션 주의 — SWIFTDATA_MIGRATION.md), freq×severity 미입력·3단계 미선택 = notAssessed, 저장/표시/PDF/집계(대시보드 due·분포)에서 notAssessed 처리, canSave 규칙 재검토. **안전 의미 역전 방지 = 최우선.** /tdd.
- **LEGAL-QUICK**: TBM 키워드 메타데이터 제거(심사 2.3.7 오해 유발 방지) · 홈 "정식 위험성평가 기록" → "위험성평가 기록"(과장 완화). 문서/문자열만, 즉시.

## 법규 정합 (신뢰출처 확정 후 구현 — 순서상 검색/통계/달력과 병행 가능)
- **LEGAL-1**: 언어↔관할 분리. AppLanguage와 RegionProfile 디커플. 온보딩/설정에서 **관할(Korea / U.S. Federal) + 업종 별도 저장**. 영어+한국관할, 한국어+미국관할 조합 허용.
- **LEGAL-2 (한국, 중규모·법자문 선행)**: 산안법 36조/시행규칙(2026-06-01 강화) 요구 = **신뢰출처로 요구필드 먼저 확정** → 참여 근로자·근로자대표 참여·결과 공유 이력·개선대책 이행결과·3년 보존. 모델 확장(마이그레이션). ⚠️ 법해석 임의 금지.
- **LEGAL-3 (조치이행 갱신)**: 저장 평가 상세를 편집가능화 — 조치상태·실제 이행결과·개선후 위험성(`postRiskLevel` 입력·표시·PDF 경로). Detail 읽기전용 해제. (위험성평가의 핵심 루프.)
- **LEGAL-4 (미국, 법자문 선행)**: JHA 365일 '기한초과' 재검토(연방 일반의무 아님 — 개별기준). 일반산업(1910)/건설(1926) 템플릿 분리, State Plan 경고. ⚠️ 미국 안전실무 지식 필요, 신뢰출처 확정 선행.
- **LEGAL-5 (TBM 결정)**: 실제 Safety Briefing/TBM 모듈 구현 or 키워드 영구 제거(LEGAL-QUICK에서 임시 제거). 연방 공통주기 의무 없음, 관할별 상이.
- **LEGAL-6 (마감)**: "정식"·Global·TBM 등 전 문구 정합 + 전문가 최종 검토. (법률 자문·준수 인증 아님 명시 유지.)

## ⚠️ 이 트랙의 최대 리스크 (플래너 명시)
#2·#5·#6은 **구현이 아니라 "정확한 법 요구사항 확정"이 병목**. 추측으로 필드/템플릿 만들면 "법 준수 앱"이라 광고하다 **미준수로 더 큰 문제**. 각 LEGAL-2/4는 **신뢰출처(법령 원문·전문가) 확정 게이트를 통과한 뒤** 구현 착수. 면책 고지("기록 도구·판정 아님")는 계속 유지(방어선).

## 🗺️ 전체 출시 로드맵 (v2 = 대규모 2차개발, 분기 단위 · 오너 인지)
LEGAL-0(즉시) → WO-A/B/C(결제) → ASC 등록 → WO-15~18(Mac UX) → WO-19(검색)/20(통계)/21(달력) → LEGAL-1~6(정합, 법자문 게이트) → 마감(스샷·문서·제출). **income 지연 트레이드오프 오너 확정.**

---

# ⚖️✎ 법규 트랙 정정 (검토자 2차 검수 수용, 2026-07-13)
...
관할·업종·참여자·개선후위험도 필드가 나중에 바뀌면 검색인덱스·통계집계·Mac표를 재작업. → **도메인 모델 먼저 안정화.**
```
...
LEGAL-2      한국 평가·조치이행·참여·공유·3년보존 (+ 개인정보 게이트 ↓)
LEGAL-4      미국 JHA 재검토 + 일반산업/건설 템플릿 분리 + State Plan 경고
LEGAL-5/TBM  TBM 실모듈 구현 (오너 확정 "무조건 구현" — 제품기능으로 정식 편입)
──── 도메인 안정 후 ────
WO-A/B/C(결제, 별도 워크스페이스 권장) · WO-15~18(Mac) · WO-19(검색)/20(통계)/21(달력)
LEGAL-6      최종 전문가 검토·문구·스샷·제출
```
> ⚠️ 결제(WO-A/B/C)는 병행 가능하되 **같은 공유 워킹카피 동시 병렬 지양**(브랜치 충돌).

## 🔴 신규 게이트 — 개인정보 재검토 (검토자 캐치, 플래너 누락했던 것)
LEGAL-2에서 **근로자·근로자대표 이름을 CloudKit에 추가하면 App Privacy "Data Not Collected" 결론이 깨질 수 있음.** WO-6 개인정보 문서(privacy_policy·app_privacy_answers) 재검토 필수. **최소화 우선**: 이름 대신 **역할·인원수·참여확인 방식**으로. 기존 "수집 0" 자동 유지 금지.
